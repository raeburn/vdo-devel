...
static struct int_map        *allocated_map;
...
static void initVIO(struct reference_block *block, struct vio *vio)
{
  ...
  /*
   * Hack! int_map takes a pointer value but we want to store an int, so we
   * cast; int_map doesn't like NULL so we set a bit outside the low 32 where
   * our value is stored.
   */
  VDO_ASSERT_SUCCESS(vdo_int_map_put(allocated_map, (unsigned long)vio,
				     (void *)((0x1ULL << 32) | allocated_count),
				     true, NULL));
}
...
static void doIngest(struct vdo_completion *completion)
{
  ...
  for (int iter = 0; iter < iteration_count; iter++) {
    for (int i = 0; i < vio_count; i++) {
      ...
      void *map_value = vdo_int_map_get(allocated_map, (unsigned long)vio);
      u32 expected_alloc_count = (u32)(unsigned long)map_value;
...
static void testBasic(void)
{
  ...
  VDO_ASSERT_SUCCESS(vdo_int_map_create(0, &allocated_map));
  ...
  vdo_int_map_free(allocated_map);
  allocated_map = NULL;
  free(wrappers);
}
